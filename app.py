...
def generate_ai_cheer(team: str, score_old: int, score_new: int, api_key: str) -> str:
    ...
    prompt = (
        ...
        f"- '세기말' 팀은 90년대~00년대 레트로 감성(인소 말투, 옛날 유행어 등)으로 작성\n"
        f"- '새천년' 팀은 최신 MZ세대 유행어(폼 미쳤다, 가보자고, 찢었다, 폼생폼사, 알잘딱깔센 등)로 작성\n"
        f"- [중요] 특정 밈(예: 럭키비키)만 반복해서 남발하지 말고, 상황에 맞게 매번 새롭고 다양한 드립을 창작할 것!\n"
        f"- 25자 이내로 짧고 강렬하게\n"
        f"- 이모지 1~2개 포함 (🔥⚾🏆💪🎉👏🙌😎💥 중에서)\n"
        f"- 설명, 인사말, 따옴표 없이 오직 멘트 텍스트만 출력"
    )
    resp = requests.post(
...
if _bdata.get("broadcast"):
    ...
    # ── 오버레이: st.markdown으로 메인 페이지에 직접 렌더링 (onclick 안정적 동작)
    st.markdown(f"""
<style>
@keyframes bshow_{_bc_id}{{0%{{opacity:0;transform:scale(.88)}}12%{{opacity:1;transform:scale(1)}}82%{{opacity:1;transform:scale(1)}}100%{{opacity:0;transform:scale(.95)}}}}
@keyframes bpulse_{_bc_id}{{0%,100%{{transform:scale(1)}}50%{{transform:scale(1.04)}}}}
#bcov_{_bc_id}{{
    position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:99999;
    background:{_bc_bg};
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    cursor:pointer;user-select:none;-webkit-user-select:none;
    animation:bshow_{_bc_id} 8s ease-in-out 1 forwards;
    font-family:'Noto Sans KR',sans-serif;
}}
#bcov_{_bc_id} .bc-badge{{
    color:#fff;border:3px solid {_bc_color};border-radius:10px;
    padding:8px 28px;font-size:clamp(1rem,3.5vw,2rem);font-weight:900;
    margin-bottom:36px;background:rgba(0,0,0,.4);letter-spacing:2px;
    box-shadow:0 0 12px {_bc_color},0 0 28px {_bc_color};text-shadow:0 0 6px #fff;
}}
#bcov_{_bc_id} .bc-msg{{
    color:#fff;font-size:clamp(2rem,8vw,5.5rem);font-weight:700;
    letter-spacing:4px;text-align:center;padding:0 40px;line-height:1.3;
    text-shadow:0 0 4px #fff,0 0 20px {_bc_color},0 0 60px {_bc_color};
    animation:bpulse_{_bc_id} 1.6s ease-in-out infinite;
}}
#bcov_{_bc_id} .bc-hint{{
    position:absolute;bottom:40px;
    color:rgba(255,255,255,.45);font-size:clamp(.75rem,2.5vw,1rem);letter-spacing:1px;
}}
</style>
<div id="bcov_{_bc_id}"
     onclick="(function(){{var el=document.getElementById('bcov_{_bc_id}');if(el)el.style.display='none';try{{sessionStorage.setItem('bcast_done_{_bc_id}','1');}}catch(e){{}}}})()"
     ontouchstart="(function(e){{e.preventDefault();var el=document.getElementById('bcov_{_bc_id}');if(el)el.style.display='none';try{{sessionStorage.setItem('bcast_done_{_bc_id}','1');}}catch(e){{}}}})(event)">
  <div class="bc-badge">{_bc_label}</div>
  <div class="bc-msg">{_bc_msg}</div>
  <div class="bc-hint">화면을 터치하면 닫힙니다</div>
</div>
""", unsafe_allow_html=True)
    # ── 효과음 + sessionStorage 복원 (이미 닫은 경우 즉시 숨김)
    components.html(f"""<script>
(function() {{
    var bid = '{_bc_id}';
    ...
    var hide = function() {{
        try {{
            var el = parent.document.getElementById('bcov_' + bid);
            if (el) el.style.display = 'none';
        }} catch(e) {{}}
    }};

    // 이미 닫은 경우 즉시 숨김
    if (s && s.getItem(keyDone)) {{ hide(); return; }}
    ...
    try {{
        ...
        var play = function() {{
            if (bcTeam === '공지') {{
                ...
                [[523.25,0.00,0.20],[659.25,0.22,0.20],[784.00,0.44,0.20],[1046.5,0.66,1.00]].forEach(function(n) {{
...
# ── TAB 3: 3행시 이벤트 ──────────────────────────────────────
HAIKU_WORDS = {"세기말": ["세", "기", "말"], "새천년": ["새", "천", "년"], "발야구": ["발", "야", "구"]}
HAIKU_VALID = {  # 각 줄에서 허용하는 첫 글자 목록
    "세기말": [["세"], ["기"], ["말"]],
    "새천년": [["새"], ["천"], ["년", "연"]],
    "발야구": [["발"], ["야"], ["구"]],
}
...
with tab3:
    ...
    <div style="text-align:center;color:#aaa;font-size:0.9rem;margin-bottom:20px;">
        '세기말' · '새천년' · '발야구' 중 하나로 3행시를 짓고 하트를 받으세요!<br>
        ❤️ 가장 많은 하트를 받은 분께 <b style="color:#ffcc44;">선물</b>을 드립니다 🎁
    </div>""", unsafe_allow_html=True)

    # ── 작성 폼 ──
    with st.expander("✏️ 3행시 짓기", expanded=True):
        hw = st.radio("주제 선택", ["세기말", "새천년", "발야구"], key="haiku_word_radio", horizontal=True)
        syls = HAIKU_WORDS[hw]
        valid = HAIKU_VALID[hw]
        wc = "#ff8844" if hw == "세기말" else ("#88ee55" if hw == "발야구" else "#44ccff")
        hint_syls = ["/".join(v) for v in valid]
        st.markdown(f'<div style="color:{wc};font-size:0.85rem;margin:4px 0 8px;">각 줄의 첫 글자: <b>{hint_syls[0]}</b> · <b>{hint_syls[1]}</b> · <b>{hint_syls[2]}</b></div>', unsafe_allow_html=True)
        hl1 = st.text_input(f"{hint_syls[0]} —", placeholder=f"'{hint_syls[0]}'(으)로 시작하는 첫 번째 줄", key="haiku_l1")
        hl2 = st.text_input(f"{hint_syls[1]} —", placeholder=f"'{hint_syls[1]}'(으)로 시작하는 두 번째 줄", key="haiku_l2")
        hl3 = st.text_input(f"{hint_syls[2]} —", placeholder=f"'{hint_syls[2]}'(으)로 시작하는 세 번째 줄", key="haiku_l3")
        if st.button("💌 3행시 등록", use_container_width=True, type="primary", key="haiku_submit"):
            _errs = []
            for _line, _vlist, _hint, _n in [(hl1.strip(), valid[0], hint_syls[0], "첫"), (hl2.strip(), valid[1], hint_syls[1], "두"), (hl3.strip(), valid[2], hint_syls[2], "세")]:
                if not _line or not any(_line.startswith(s) for s in _vlist):
                    _errs.append(f"{_n} 번째 줄은 **'{_hint}'** 으로 시작해야 합니다.")
            if _errs:
                for _e in _errs:
                    st.error(_e)
            else:
    ...
    if not _hlist:
        ...
        for _h in _hlist:
            _tc = "#ff4499" if _h["team"] == "세기말" else "#66ccff"
            _wc2 = "#ff8844" if _h["word"] == "세기말" else ("#88ee55" if _h["word"] == "발야구" else "#44ccff")
